...
- Step 2: 新建一个 mesh0，使用 `read_uds` 节点导入网格文件，直接连接到 `write_usd` 节点上可视化以确保导入成功。我们提供的网格文件在 [data/](../data/) 文件夹下。在导入这个 grid1x1.usda 文件之后，因为角度关系，可能需要调整视角（Q\E上下移动相机）才能看到场景中的物体。我们也提供了更高分辨率的网格。
