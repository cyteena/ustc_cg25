...
class Seamless : public CloneMethod
{
   ...
   private:
   void build_poisson_equation();

    void precompute_matrix();

    void solve_channel(int channel);

    // LDLT: lower Diagonal lower transpose (suitable for symmetrix positive-definite sparse)
    // decomposes the matrix into a product of a lower triangular matrix (L), a diagonal matrix (D) and L^T
